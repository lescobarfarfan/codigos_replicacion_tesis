...
for ar in tqdm(archivos):
    ...
    # Layouts
    l = gt.random_layout(red)
    l2 = gt.sfdp_layout(red, C=100000, p=12, gamma=15)
    l3 = gt.fruchterman_reingold_layout(red, a=4, circular=True, r=8)

    lSinDesc = gt.random_layout(red_sinDesc)
    l2SinDesc = gt.sfdp_layout(red_sinDesc, C=100000, p=13, gamma=15)
    l3SinDesc = gt.fruchterman_reingold_layout(red_sinDesc, a=4,
                                               circular=True, r=8)

    # Graficamos
    nombreRed = "RedFULL_Layout1_" + fecha + ".pdf"
    gt.graph_draw(red, pos=l,
                  edge_pen_width=gt.prop_to_size(pesos, mi=1, ma=15),
                  vertex_size=gt.prop_to_size(betweenness[0], mi=1, ma=50),
                  vertex_fill_color=pagerank,
                  vertex_halo=True,
                  vertex_halo_size=1,
                  edge_color=red.edge_properties['weight'],
                  vcmap=matplotlib.cm.gist_heat,
                  output_size=(1000, 1000),
                  output=nombreRed)

    nombreRed = "RedFULL_Layout2_" + fecha + ".pdf"
    gt.graph_draw(red, pos=l2,
                  edge_pen_width=gt.prop_to_size(pesos, mi=1, ma=15),
                  vertex_size=gt.prop_to_size(betweenness[0], mi=1, ma=50),
                  vertex_fill_color=pagerank,
                  vertex_halo=True,
    ...
    nombreRed = "RedFULL_Layout3_" + fecha + ".pdf"
    try:
    	gt.graph_draw(red, pos=l3,
                  edge_pen_width=gt.prop_to_size(pesos, mi=1, ma=15),
                  vertex_size=gt.prop_to_size(betweenness[0], mi=1, ma=50),
                  vertex_fill_color=pagerank,
                  vertex_halo=True,
    	...
    	print("El archivo " + nombreRed + " no pudo ser creado.\n")


    # Ahora, la red sin nodos desconectados
    nombreRed = "RedSinDesc_Layout1_" + fecha + ".pdf"
    try:
    	gt.graph_draw(red_sinDesc, pos=lSinDesc,
    				  edge_pen_width=gt.prop_to_size(pesosSinDesc, mi=1, ma=15),
    		          vertex_size=gt.prop_to_size(betweennessSinDesc[0], mi=1,
    		          							  ma=50),
    		          vertex_fill_color=pagerankSinDesc,
    		          vertex_halo=True,
    		          vertex_halo_size=1,
    		          edge_color=red_sinDesc.edge_properties['weight'],
    		          vcmap=matplotlib.cm.gist_heat,
    		          output_size=(1000, 1000),
    		          output=nombreRed)
    except:
    ...
    nombreRed = "RedSinDesc_Layout2_" + fecha + ".pdf"
    try:
    	gt.graph_draw(red_sinDesc, pos=l2SinDesc,
    				  edge_pen_width=gt.prop_to_size(pesosSinDesc, mi=1, ma=15),
    				  vertex_size=gt.prop_to_size(betweennessSinDesc[0], mi=1,
                                                 ma=50),
    				  vertex_fill_color=pagerankSinDesc,
    ...
    nombreRed = "RedSinDesc_Layout3_" + fecha + ".pdf"
    try:
    	gt.graph_draw(red_sinDesc, pos=l3SinDesc,
    				  edge_pen_width=gt.prop_to_size(pesosSinDesc, mi=1, ma=15),
    				  vertex_size=gt.prop_to_size(betweennessSinDesc[0], mi=1,
                                             	  ma=50),
    				  vertex_fill_color=pagerankSinDesc,
    ...
    nombreSBM = "AjusteSBM_FULL_Layout1_NDC_" + fecha + ".pdf"
    try:
    	best_NDC.draw(pos=l, vertex_halo=True, vertex_halo_color="black",
                  vertex_halo_size=1, output_size=(1000, 1000),
                  output=nombreSBM)
    except:
    	print("El archivo " + nombreSBM + " no pudo ser creado.\n")


    nombreSBM = "AjusteSBM_FULL_Layout2_NDC_" + fecha + ".pdf"
    try:
    	best_NDC.draw(pos=l2, vertex_halo=True, vertex_halo_color="black",
                  vertex_halo_size=1, output_size=(1000, 1000),
                  output=nombreSBM)
    except:
    	print("El archivo " + nombreSBM + " no pudo ser creado.\n")
    ...
    nombreSBM = "AjusteSBM_FULL_Layout3_NDC_" + fecha + ".pdf"
    try:
    	best_NDC.draw(pos=l3, vertex_halo=True, vertex_halo_color="black",
                  vertex_halo_size=1, output_size=(1000, 1000),
                  output=nombreSBM)
    except:
    	print("El archivo " + nombreSBM + " no pudo ser creado.\n")
    ...
    nombreSBM = "AjusteSBM_SinDesc_Layout1_NDC_" + fecha + ".pdf"
    try:
    	bestSinDesc_NDC.draw(pos=lSinDesc, vertex_halo=True,
    ...
    nombreSBM = "AjusteSBM_SinDesc_Layout2_NDC_" + fecha + ".pdf"
    try:
    	bestSinDesc_NDC.draw(pos=l2SinDesc, vertex_halo=True,
                         vertex_halo_color="black",
                         vertex_halo_size=1, output_size=(1000, 1000),
                         output=nombreSBM)
    except:
    ...
    nombreSBM = "AjusteSBM_SinDesc_Layout3_NDC_" + fecha + ".pdf"
    try:
    	bestSinDesc_NDC.draw(pos=l3SinDesc, vertex_halo=True,
                         vertex_halo_color="black",
                         vertex_halo_size=1, output_size=(1000, 1000),
                         output=nombreSBM)
    except:
    ...
    if best_DC.B > 1:
        ...
        nombreSBM = "AjusteSBM_FULL_Layout2_DC_" + fecha + ".pdf"
        best_DC.draw(pos=l2, vertex_halo=True,
                     vertex_halo_color="black",
                     vertex_halo_size=1,
                     output_size=(1000, 1000),
                     output=nombreSBM)

        nombreSBM = "AjusteSBM_FULL_Layout3_DC_" + fecha + ".pdf"
        best_DC.draw(pos=l3, vertex_halo=True,
                     vertex_halo_color="black",
                     vertex_halo_size=1,
                     output_size=(1000, 1000),
                     output=nombreSBM)
    ...
    if bestSinDesc_DC.B > 1:
        ...
        nombreSBM = "AjusteSBM_SinDesc_Layout2_DC_" + fecha + ".pdf"
        bestSinDesc_DC.draw(pos=l2SinDesc, vertex_halo=True,
                            vertex_halo_color="black",
                            vertex_halo_size=1,
                            output_size=(1000, 1000),
                            output=nombreSBM)

        nombreSBM = "AjusteSBM_SinDesc_Layout3_DC_" + fecha + ".pdf"
        bestSinDesc_DC.draw(pos=l3SinDesc, vertex_halo=True,
                            vertex_halo_color="black",
                            vertex_halo_size=1,
                            output_size=(1000, 1000),
                            output=nombreSBM)
